...
import { LogOut, Trash2, Plus, Image as ImageIcon, Layout } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const fetchGalleryLayout = async () => {
    ...
    if (!error && data) {
      setCurrentLayout(data.layout_type as LayoutType);
    } else {
      setCurrentLayout('grid');
    }
  };

  const updateGalleryLayout = async (layoutType: LayoutType) => {
    ...
    if (error) {
      setMessage('Error updating layout');
    } else {
              ...
              <div className="mb-6 border-2 border-gray-800 p-6">
                ...
                <div className="flex flex-wrap gap-4">
                  {(['grid', 'masonry', 'collage', 'grouped'] as LayoutType[]).map((layout) => (
                    <button
                      key={layout}
                      onClick={() => updateGalleryLayout(layout)}
                      className={`px-6 py-3 border-2 uppercase tracking-wider transition-all ${
                ...
                <p className="text-sm text-gray-400 mt-3">
                  Current layout: <span className="text-white font-semibold uppercase">{currentLayout}</span>
                </p>
              </div>
